...
for filename in allfiles.files_filtered(OBSTYPE='SKY FLAT',FILTERS='OH'):
    ccd = fits.getdata(allfiles.location + filename)
    ccd = cp.subtract_overscan(ccd,overscan_axis=0,fits_section =ccd.header['BIASSEC'])
    ccd = cp.trim_image(ccd,fits_section=ccd.header['TRIMSEC'])
    ccd = cp.subtract_bias(ccd,master_bias)
    flatlist_OH.append(ccd)
master_flat_OH = cp.combine(flatlist_OH,method='average')
